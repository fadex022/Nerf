""""
Copyright © Gauss 2022 - https://github.com/fadex022 
...
class Owner(commands.Cog, name="owner"):
    ...
    async def sync(self, context: Context, scope: str) -> None:
        ...
        if scope == "global":
            ...
            embed = discord.Embed(
                title="Commandes Slash Sync",
                description="Les commandes de Slash ont été synchronisées dans cette guilde.",
                color=0x9C84EF
            )
            await context.send(embed=embed)
            return
        embed = discord.Embed(
            title="Champ d'application non valide",
            description="La portée doit être `global` ou `guild`.",
            color=0xE02B2B
        )
        await context.send(embed=embed)
    ...
    @app_commands.describe(scope="La portée de la synchronisation. Peut être `global`, `current_guild` ou `guild`.")
    @checks.is_owner()
    async def unsync(self, context: Context, scope: str) -> None:
        ...
        if scope == "global":
            ...
            await context.bot.tree.sync()
            embed = discord.Embed(
                title="Commandes Slash Unsync",
                description="Les commandes Slash ont été globalement désynchronisées.",
                color=0x9C84EF
            )
            await context.send(embed=embed)
            return
            ...
            embed = discord.Embed(
                title="Commandes Slash Unsync",
                description="Les commandes de Slash ont été désynchronisées dans cette guilde.",
                color=0x9C84EF
            )
            await context.send(embed=embed)
            return
    ...
    @app_commands.describe(cog="Le nom de la cog à charger")
    @checks.is_owner()
    async def load(self, context: Context, cog: str) -> None:
        ...
        :param cog: Le nom de la cog à charger.
        """
        try:
            ...
            embed = discord.Embed(
                title="Erreur!",
                description=f"Impossible de charger le `{cog}` cog.",
                color=0xE02B2B
            )
            await context.send(embed=embed)
            return
        embed = discord.Embed(
            title="Load",
            description=f"On a réussi à charger le `{cog}` cog.",
            color=0x9C84EF
        )
        await context.send(embed=embed)
